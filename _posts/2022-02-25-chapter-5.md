...
The decomposition ([\[eq:cp\]](#eq:cp){reference-type="ref"
reference="eq:cp"}) is known by several names reflecting its history,
...
and Hitchcock decomposition [@kolda09]. The idea is to build up
...
{% raw %}
$\phi_c(\sublist{x}{{\mathcal{D}}})$
{% endraw %} 
over
${\mathcal{D}}$ variables, ([\[eq:cp\]](#eq:cp){reference-type="ref"
reference="eq:cp"}) is equivalent to decomposing $\phi_c$ into a sum of
...
\begin{align}
    ...
    &= \sum_{k=1}^{r} w_k^c\; \psi_{k,1}^c(x_1) \psi_{k,2}^c(x_2) \cdots \psi_{k,{\mathcal{D}}}^c(x_{\mathcal{D}}). \label{eq:fact}
\end{align}
$$
{% endraw %}
...
([\[eq:cp\]](#eq:cp){reference-type="ref" reference="eq:cp"}) always
...
fix $r=1$ in ([\[eq:cp\]](#eq:cp){reference-type="ref"
reference="eq:cp"}), use numerical methods to find a rank-$r$
...
approximation. This problem is stated as $$\begin{aligned}
    \min_{\mathbf{T}_r} \Vert(\mathbf{T}_r, \mathbf{T})\Vert_F^2 \quad \textrm{such that} \quad \mathbf{T}_r = \sum_{k=1}^r w_k \; \mathbf{a}_k^1 \otimes \cdots \otimes \mathbf{a}_k^{\mathcal{D}} \label{eq:cp_opt}\end{aligned}$$
...
optimal solution [@kolda09; @nickthesis].

Note, however, that even if we could solve
([\[eq:cp_opt\]](#eq:cp_opt){reference-type="ref"
reference="eq:cp_opt"}) optimally, this does not necessarily lead to the
...
Specifically, solving $$\begin{aligned}
    2ab &= \theta \\
    a^2 + b^2 &= \frac{1}{\theta}\end{aligned}$$ gives $$\begin{aligned}
    ...
    a &= \frac{1}{2} \left( -\sqrt{\theta+\frac{1}{\theta}} \pm \sqrt{\theta-\frac{1}{\theta}} \right), & b &= \frac{1}{2} \left( -\sqrt{\theta+\frac{1}{\theta}} \mp \sqrt{\theta-\frac{1}{\theta}} \right) \label{eq:symm_decomp}.\end{aligned}$$
...
## Operations using decomposed functions
...
#### Rank decomposition as a probability distribution
...
## Low-rank tensor propagation {#low-rank-tensor-propagation}
...
### Consistency of tensor propagation
...
## Reweighting of decomposed functions

### Max-norm reweighting {#sec:maxnorm}

Let $\phi$ be a potential function represented in decomposed form, i.e.
$$\begin{aligned}
    \phi_c(\mathbf{x}) &= \sum_k w_k\; \psi_{k}(\mathbf{x}).
%     &= \sum_{k=1}^{r} w_k^c\; \psi_{k,1}^c(x_1) \psi_{k,2}^c(x_2) \cdots \psi_{k,d}^c(x_d). \label{eq:fact}\end{aligned}$$

We are free to choose any new set of weights $\{w'_k\}$ that satisfy
$\sum_kw'_k=1$, by reweighting the $\psi_k$: $$\begin{aligned}
    \phi_c(\mathbf{x}) &= \sum_k w_k\; \psi_{k}(\mathbf{x}) \nonumber \\
