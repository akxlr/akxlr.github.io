...
### 5.4.2 Consistency of tensor propagation {#sec:tensor_prop}
...
Let $\phi_1$ and $\phi_2$ be $M$-bounded
...
Suppose we have $F$ $M$-bounded functions
...
greater than $3Mt$ since we can invoke [Lemma 5.4.2](#lem:542) with
$\beta=1$. If the error after $F$ multiplications, $r_F'-r_F$, is no
greater than $(2^F-1)M^{F-1}t$, then the error after $F+1$
multiplications, $r'_{F+1}-r_{F+1}$, is bounded by invoking 
[Lemma 5.4.2](#lem:542) with
...
$\textsc{Multiply}$. By [Corollary 5.4.2](#corr:542), if
...
has error no more than $(2M)^Ct$ by [Corollary 5.4.2](#corr:542).
...
$$\left| \overline Y_j - \mathbb{E}{[}{\overline Y_j}{]} \right| \leq t  \; \textrm{for all estimates $j$} \implies \vert \hat d_i(x_i) - d_i(x_i) \vert \leq 2^{TQ}(2M)^{Q^2+C}t \; \textrm{for all}\; i, x_i.$$

Therefore, by (\ref{eq:h_eps_bound}), 
...
%     \implies &
    P\left(\vert \hat d_i(x_i) - d_i(x_i) \vert \geq 2^{TQ}(2M)^{Q^2+C}t \; \textrm{for any}\; i, x_i\right) \leq \alpha C^2 2^T \exp \left(-\frac{2Kt^2}{M^2}\right)
\end{aligned}$$

To complete the proof, suppose $\epsilon$ is given and let
$t = \frac{\epsilon}{2^{TQ}(2M)^{Q^2+C}}$. Then 

$$
\begin{align}
    P\left(\vert \hat d_i(x_i) - d_i(x_i) \vert \geq \epsilon \; \textrm{for any} \; i, x_i\right) \leq \alpha C^2 2^T \exp \left(-\frac{2K\epsilon^2}{M^2 2^{2TQ}(2M)^{2(Q^2+C)}}\right). \tag{5.9}\label{eq:h_bound}
\end{align}
$$
...
The bound (\ref{eq:h_bound}) is sufficient to prove consistency of TP, but
...
(Multiplicative Chernoff bound). Let $\sublist{Y}{K}$ be iid random variables in range $[0,M]$, and let
$\overline Y = \frac{1}{K}\sum_{i=1}^K Y_i$. For any small $t>0$,
{:.theorem text='5.4.3' id='theo:543'}

$$\begin{aligned}
    P\left(\overline Y \leq (1-t)\mathbb{E}{[}{\overline Y}{]}\right) &\leq \exp \left(-\frac{t^2\mathbb{E}{[}{\overline Y}{]} K }{2M}\right),\;\textrm{and} \\
    P\left(\overline Y \geq (1+t)\mathbb{E}{[}{\overline Y}{]}\right) &\leq \exp \left(-\frac{t^2\mathbb{E}{[}{\overline Y}{]} K }{3M}\right)\end{aligned}$$

and therefore 

$$\begin{align}
    P\left(\overline Y \notin \left[(1-t)\mathbb{E}{[}{\overline Y}{]}, (1+t)\mathbb{E}{[}{\overline Y}{]}\right] \right) &\leq 2 \exp \left(-\frac{t^2\mathbb{E}{[}{\overline Y}{]} K }{3M}\right). \tag{5.10}\label{eq:chernoff}
\end{align}
$$
...
Consider the above problem setup
([Section 5.4.1](#sec:analysis_setup)), and assume that the expected value of
any table cell estimate during the TP procedure is lower bounded by
$B>0$. With probability at least $1-\delta$,
{:.theorem text='5.4.4' id='theo:544'}

$$(1-\epsilon)d_i(x_i)\leq \hat d_i(x_i) \leq (1+\epsilon)d_i(x_i)$$

for all $i$ and $x_i$, if the sample size used for all multiplication
operations is at least

$$K_{\min}(\epsilon, \delta) \in \bigO{\frac{C^2}{\epsilon^2}\frac{M}{B}\left(\log C + T + \log\frac{1}{\delta}\right)}.$$


*Proof.* Our proof is similar to the proof of [Theorem 5.4.2](#th:542). The $\bigO{C^2 2^T}$ estimates from [Corollary 5.4.1](#corr:541) each satisfy the Chernoff bound with respect to
their expected value, and applying a union bound to these gives

$$
\begin{align}
    P_{t}\left(\overline Y_j \notin \left[(1-t)\mathbb{E}{[}{\overline Y_j}{]}, (1+t)\mathbb{E}{[}{\overline Y_j}{]}\right]  \; \textrm{for any estimate $j$} \right) \leq \alpha_1 C^2 2^T \exp \left(-\frac{t^2 B K }{3M}\right) \tag{5.11}\label{eq:c_epsbound}
\end{align}
$$

where $\overline Y_j$ denotes the estimate made by the
$\textsc{Multiply}$ procedure of some table cell $j$, and we
introduce constant $\alpha_1$ as above. Note that we have replaced
$\mathbb{E}{[}{\overline Y}{]}$ with the smaller $B$ on the RHS.

We claim that
$\overline Y_j \in \left[(1-t)\mathbb{E}{[}{\overline Y_j}{]}, (1+t)\mathbb{E}{[}{\overline Y_j}{]}\right]$
...
$\overline Y_j = (1+t)\mathbb{E}{[}{\overline Y_j}{]}$, and examining how the error
...
on the number of $\textsc{Multiply}$ calls made throughout the
...
$\alpha_2 C$ $\textsc{Multiply}$ calls, we get an extra factor of
...
$t =  {}^{\log(1+\epsilon)} {\mskip -5mu/\mskip -3mu}_ {\alpha_2 C}$ implies
$(1-t)^{\alpha_2 C} \geq 1-\epsilon$ and
$(1+t)^{\alpha_2 C} \leq (1+\epsilon)$: 

$$
\begin{align}
    (1-t)^{\alpha_2 C} = \left(1-\frac{\log(1+\epsilon)}{\alpha_2 C}\right)^{\alpha_2 C} &\geq \left(1 - \frac{\epsilon}{\alpha_2 C}\right)^{\alpha_2 C} \geq 1 - \epsilon \tag{5.12}\label{eq:eps1}\\
    (1+t)^{\alpha_2 C} = \left(1+\frac{\log(1+\epsilon)}{\alpha_2 C}\right)^{\alpha_2 C} &\leq \exp\left(\log(1+\epsilon)\right) = 1+\epsilon. \tag{5.13}\label{eq:eps2}
\end{align}
$$
...
(\ref{eq:c_epsbound}), we obtain 

$$
\begin{align}
    P_{t}\left(\hat d_i(x_i) \notin \left[(1-\epsilon)d_i(x_i), (1+\epsilon)d_i(x_i)\right]\right) \leq \alpha_1 C^2 2^T \exp \left(-\frac{BK\log^2(1+\epsilon)}{3M\alpha_2^2C^2}\right) \tag{5.14}\label{eq:514}
\end{align}
$$
...
\begin{aligned}
    ...
    \implies &K \geq \frac{3M\alpha_2^2C^2}{B\log^2(1+\epsilon)}\log\left(\frac{\alpha_1C^22^T}{\delta}\right) =: K_{\min}(\epsilon,\delta).
\end{aligned}
$$
...
[Theorem 5.4.4](#theo:544) gives us (an asymptotic upper bound on) the
...
(\ref{eq:cp_marg}). The junction tree algorithm, however, will
...
reason, [Theorem 5.4.4](#theo:544) cannot be used to establish consistency in the
general case.
...
\begin{align}
    ...
    &= \sum_k w'_k \psi'_{k}(\mathbf{x}),\quad \psi'_k(\mathbf{x}) = \frac{w_k}{w'_k} \psi_{k}(\mathbf{x}). \tag{5.15}\label{eq:reweighting}
\end{align}
$$
...
maximum value. Specifically, we set 

$$
\begin{align}
    \psi"_k(\mathbf{x}) &= \frac{\psi_k(\mathbf{x})}{\max_\mathbf{x}\psi_k(\mathbf{x})} \nonumber \\
    w"_k &= w_k \cdot {\max_\mathbf{x}\psi_k(\mathbf{x})} \tag{5.16}\label{eq:maxnorm}
\end{align}
$$
...
\begin{aligned}
    ...
    \psi'_k(\mathbf{x}) &= \psi"_k \cdot \sum_l w"_l.
\end{aligned}
$$
...
performance (see [Chapter 6]({% link _posts/2022-02-26-chapter-6.md %})).
...
(\ref{eq:reweighting}) that minimises the sum of the variance of
...
\begin{align}
    ...
    &\implies w'_k = \frac{w_k\sqrt{\sum_\mathbf{x}\psi_k(\mathbf{x})^2}}{\sum_l w_l\sqrt{\sum_\mathbf{x}\psi_l(\mathbf{x})^2}}, \tag{5.17}\label{eq:minvar}
\end{align}
$$
{% endraw %}
...
(\ref{eq:cp_marg}), with the additional step of performing an
...
Notably, [{{page.can00}}] describe "Penniless propagation", where they use
...
representations also includes [{{page.sta00}}], who use algebraic decision
diagrams (ADDs)[^15] in the context of solving Markov decision processes
(MDP), in an algorithm they call APRICODD.

Recently, [{{page.gog13}}] formalised to this idea via a framework they call
...
Other recent work includes [{{page.sanner2012symbolic}}], who use piecewise
...
work includes [{{page.daw94}}], where the authors describe "hybrid propagation",
...
further extended by [{{page.kja95}}].
...
[{{page.kol99}}]. This paper explains how one can maintain compact
...
approach for both discrete and continuous distributions. [{{page.sud03}}] offer
...
Continuing this theme, [{{page.ihl09}}] describe "particle belief propagation"
...
graphs only). More recent work in this area includes [{{page.eli12}}; {{page.lie15}}].
...
gene clustering [{{page.pha11}}]. Notably, [{{page.sav06}}] have investigated the tensor
...
[^1]: In general, (\ref{eq:cp_opt}) may have no optimal solution, because there
...
[^9]: We can construct an example where $M/B$ grows exponentially with
    ...
    $\phi(x, x') = \unicode{x1D7D9}[x == x'] + 1$ and constant unary
