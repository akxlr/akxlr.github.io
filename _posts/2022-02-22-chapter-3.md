...
# Exact inference {#cha:exact}

In this chapter we describe how the structure encoded by the PGMs
defined in Chapter [\[cha:pgms\]](#cha:pgms){reference-type="ref"
reference="cha:pgms"} can be used to efficiently perform exact
...
[\[cha:approximate\]](#cha:approximate){reference-type="ref"
reference="cha:approximate"} and
[\[cha:tp\]](#cha:tp){reference-type="ref" reference="cha:tp"}. We
...
which we define in Section
[1.3.1.1](#sec:triangulation){reference-type="ref"
reference="sec:triangulation"}.

## Elimination algorithm {#sec:jt_structure}

The *elimination algorithm* (Algorithm
[\[alg:elimination\]](#alg:elimination){reference-type="ref"
reference="alg:elimination"}) operates on factor graphs and is the
...
complexity described in Definition
[\[def:marg\]](#def:marg){reference-type="ref" reference="def:marg"} by
reordering sums and products: $$\begin{aligned}
    p(a) &\propto \sum_b \sum_c \sum_d \phi_{AB}(a,b)\phi_{AD}(a,d)\phi_{BC}(b,c)\phi_C(c) \\
    &= \sum_b \phi_{AB}(a,b) \sum_c \phi_{BC}(b,c)\phi_C(c) \sum_d \phi_{AD}(a,d).\end{aligned}$$
...
$$\begin{aligned}
    ...
    &= \psi_1(a) \psi_3(a) & \psi_3(a) = \sum_b \phi_{AB}(a,b) \psi_2(b)\end{aligned}$$
...
order in which to eliminate variables (line
[\[line:elim_order\]](#line:elim_order){reference-type="ref"
reference="line:elim_order"} in Algorithm
[\[alg:elimination\]](#alg:elimination){reference-type="ref"
reference="alg:elimination"}). The speedup we are able to achieve
...
## Belief propagation on trees {#sec:bp}
...
## Junction tree algorithm {#sec:jt}
...
### Building the junction tree {#sec:jt_structure}
...
#### Triangulation {#sec:triangulation}
...
#### HUGIN and Shafer-Shenoy updates
