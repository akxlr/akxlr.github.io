...
p(x) &= \sum_y \sum_z p(x,y,z) \\
     ...
     \implies P(X=0) &= 0.1 + 0.3 = 0.4.\end{aligned}$$ When working
with models with thousands of variables, efficiency gains such as these
become substantial.
:::
...
$X_i$, denoted $p(x_i)$, is given by $$\begin{aligned}
    ...
    \end{aligned}$$ where $\mathbf{x}_{-i}$ denotes the set of all
$\sublist{x}{\mathcal{D}}$ excluding $x_i$. The marginal distribution
over a non-singleton set of variables $\mathbf{X}_S$ is defined
similarly:
