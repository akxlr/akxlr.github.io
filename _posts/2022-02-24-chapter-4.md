...
# Approximate inference {#cha:approximate}

For high-treewidth graphical models, exact inference using the
algorithms discussed in Chapter
[\[cha:exact\]](#cha:exact){reference-type="ref" reference="cha:exact"}
...
## Loopy belief propagation {#sec:loopy_bp}

The belief propagation algorithm for trees (Section
[\[sec:bp\]](#sec:bp){reference-type="ref" reference="sec:bp"}) can be
modified in a straightforward way to return approximate marginals for
general (non-tree) graphs. Recall that the recursion
([\[eq:bp_msg\]](#eq:bp_msg){reference-type="ref"
reference="eq:bp_msg"})
...
according to ([\[eq:bp_msg\]](#eq:bp_msg){reference-type="ref"
reference="eq:bp_msg"}) until convergence, or until a fixed iteration
limit is reached.
...
messages. Alternatively messages can be initialised randomly; [@mur99]
...
[@eli12]. For trees, loopy BP reduces to the familiar BP algorithm of
Section [\[sec:bp\]](#sec:bp){reference-type="ref" reference="sec:bp"}.
...
quite good [@mur99]. In the framework of variational inference, loopy BP
can be viewed as an attempt to obtain beliefs from some distribution
$q \approx p$ that solves $$\begin{aligned}
    \min_q \textrm{KL}(q \| p) \label{eq:min_kl}\end{aligned}$$ where
$\textrm{KL}(\cdot \| \cdot)$ is the Kullback-Leibler divergence, a
...
Briefly, computing the objective in
([\[eq:min_kl\]](#eq:min_kl){reference-type="ref"
reference="eq:min_kl"}) involves an intractable quantity known as the
...
the messages ([\[eq:bp_msg\]](#eq:bp_msg){reference-type="ref"
reference="eq:bp_msg"}) [@mur99; @koller09]. Loopy BP is therefore
mathematically principled, but this does not imply any particular
guarantee of accuracy.

[@koller09] discusses some additional heuristics to improve performance
...
([\[eq:min_kl\]](#eq:min_kl){reference-type="ref"
reference="eq:min_kl"}), but instead restricts $q$ to a class of
tractable distributions, namely those that fully factorise. That is, we
attempt to solve $$\begin{aligned}
    q_{\textrm{MF}} = \min_{q\in\textrm{FACT}} \textrm{KL}(q \| p) \quad \textrm{where} \quad \textrm{FACT} = \left\{ q \;\middle|\; q(\sublist{x}{\mathcal{D}}) = \prod_{i=1}^\mathcal{D} q_i(x_i) \right\}.\label{eq:mf}\end{aligned}$$
We can then perform inference efficiently using $q_\textrm{MF}$ instead
of $p$.[^1] Applying the method of Lagrange multipliers to
([\[eq:mf\]](#eq:mf){reference-type="ref" reference="eq:mf"}), we obtain
an iterative coordinate ascent algorithm 

{% raw %}
$$
\begin{aligned}
q_i(x_i) = \frac{1}{Z_i}\exp\left(\sum_{\phi : X_i \in \scope{(\phi)}} \mathbb{E}{_{\mathbf{X_j}\sim q}}[{\log \phi(x_i,\mathbf{x}_j)]} \right). \label{eq:mf2}
\end{aligned}
$$
{% endraw %}
...
([\[eq:mf2\]](#eq:mf2){reference-type="ref" reference="eq:mf2"}), see
[@koller09].
...
computational complexity [@koller09].
...
example, Gaussian distributions) [@bis06]. In general however, for
arbitrary high-dimensional distributions, the most effective method
known is *Markov-chain Monte Carlo* (MCMC) [@koller09; @and03].
...
The basic sequential Gibbs sampling procedure [@gem84] is as follows:
...
equal to the desired distribution $p$ [@and03; @koller09], and therefore
...
distribution. Recent work includes [@afshar2015linear], who describe
efficient Gibbs sampling algorithms for piecewise graphical models.

#### Mixing time {#sec:mixing}
